...
Lambdas are built on functional interfaces:
- **`Consumer<T>`**: Takes input but returns nothing.
- **`Supplier<T>`**: Returns output without taking input.
- **`Function<T, R>`**: Transforms input to output.
- **`Predicate<T>`**: Returns a boolean based on input.
...
### **General Use Cases for Each Concept**

1. **Asymptotic Notations**: Optimizing database search algorithms.
2. **Scope**: Managing application-wide configurations using global variables.
3. **Errors and Exceptions**: Implementing robust file-handling logic.
4. **Deep vs. Shallow Copy**: Duplicating configuration objects in applications.
5. **Lambda Functions**: Processing large datasets with Streams for parallelism. 
